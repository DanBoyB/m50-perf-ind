...
times <- format(seq.POSIXt(ISOdatetime(2015, 1, 1, 0, 0, 0), ISOdatetime(2015, 1, 1, 23, 55, 0), by = "5 min"), "%H:%M") %>% 
  data_frame(timeSeq = seq_along(.), time = .)

dirLevels <- c("N11 - CHE", "CHE - CAR", "CAR - BAL", "BAL - FIR", "FIR - N81", "N81 - N7", "N7 - N4",
               "N4 - N3", "N3 - N2", "N2 - BMN") %>% 
  append(c("M1 - BMN", "N2 - N3", "N3 - N4", "N4 - N7", "N7 - N81", "N81 - FIR", "FIR - BAL", 
...
traffic <- tbl(db, "min5_50_2015") %>% 
  ...
  mutate(dayType = as.integer(dayType),
         ref = sql("linkID || '_' || dayType || '_' || timeSeq"))
